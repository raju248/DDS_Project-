...
DECLARE
	...
	customer_name orders.customer_name%TYPE;
	customer_contact orders.customer_contact%TYPE;
	customer_address orders.customer_address%TYPE;
	
	
	product_id1 product.product_id%TYPE;
	product_id2 product.product_id%TYPE;
	product_qnt1 product.product_quantity%TYPE;
	product_qnt2 product.product_quantity%TYPE;
	
	product1_name product.product_name%TYPE;
	product2_name product.product_name%TYPE;
	
	process_order char;
	invalid_input EXCEPTION;
	not_enough_product EXCEPTION;
	
	order_idd orders.order_id%TYPE;
...
BEGIN
	...
	if process_order = 'y' then
		...
		select count(*) into order_idd from orders;
		...
		insert into orders values (order_idd, customer_name, customer_contact, customer_address, TO_CHAR(Sysdate, 'DD-MON-YYYY'));
		
		if (product1_price*product_qnt1 + product2_price*product_qnt2 >=6000) then
			insert into order_product@site1 values (order_idd, product_id1, product_qnt1);
			insert into order_product@site1 values (order_idd, product_id2, product_qnt2);
		else
		...
		if (product1_price>=6000) then
			update product@site1 set product.product_quantity@site1 = product.product_quantity@site1 - product_qnt1 where product.product_id@site1 = product_id1;
		else
		...
		if (product2_price>=6000) then
			update product@site1 set product.product_quantity@site1 = product.product_quantity@site1 - product_qnt2 where product.product_id@site1 = product_id2;
		else
